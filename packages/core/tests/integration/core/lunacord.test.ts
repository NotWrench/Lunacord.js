...
import type { LyricsProvider } from "@lunacord/core";
import {
...
type StubLyricsClient = Required<
  Pick<LyricsProvider, "getLyricsForTrack" | "markTrackActive" | "markTrackInactive">
>;

const attachStubLyricsProvider = (lunacord: Lunacord): StubLyricsClient => {
  const stub: StubLyricsClient = {
    getLyricsForTrack: mock(() => Promise.resolve({ status: "not_found" as const })),
    markTrackActive: mock(() => undefined),
    markTrackInactive: mock(() => undefined),
  };
  lunacord.lyrics(stub);
  return stub;
};
...
describe("Lunacord", () => {
  ...
  it("should build and register a plugin through the fluent builder", () => {
    ...
    const returned = lunacord
      .createPlugin({ name: "plugin-builder", version: "1.0.0", apiVersion: "2" })
      .observe(observe)
      .use();
  ...
  it("should re-emit debug events with node context via unified debug channel", () => {
    const lunacord = new Lunacord(BASE_OPTIONS);
    const node = lunacord.getNode("node-a")!;
    const events: string[] = [];

    lunacord.on("debug", (event) => {
      events.push(`${event.nodeId ?? "?"}:${event.scope}:${event.message}`);
    });
  ...
  it("should delegate lyrics requests to the managed player", async () => {
    const lunacord = new Lunacord(BASE_OPTIONS);
    attachStubLyricsProvider(lunacord);
    const [nodeA, nodeB] = lunacord.getNodes();
  ...
  it("should mark tracks active for lyrics cache on playback events", () => {
    const lunacord = new Lunacord(BASE_OPTIONS);
    const lyricsClient = attachStubLyricsProvider(lunacord);
    const node = lunacord.getNode("node-a")!;
    const track = new Track(MOCK_RAW_TRACK);
    ...
    expect(lyricsClient.markTrackActive).toHaveBeenCalledTimes(2);
    expect(lyricsClient.markTrackActive).toHaveBeenNthCalledWith(1, "guild-lyrics-cache", track);
    expect(lyricsClient.markTrackActive).toHaveBeenNthCalledWith(2, "guild-lyrics-cache", track);
  });
  ...
  it("should mark tracks inactive for lyrics cache on end and teardown events", () => {
    ...
    expect(lyricsClient.markTrackInactive).toHaveBeenCalledTimes(4);
    expect(lyricsClient.markTrackInactive).toHaveBeenNthCalledWith(1, "guild-lyrics-cache", track);
    expect(lyricsClient.markTrackInactive).toHaveBeenNthCalledWith(2, "guild-lyrics-cache");
    expect(lyricsClient.markTrackInactive).toHaveBeenNthCalledWith(3, "guild-lyrics-cache");
    expect(lyricsClient.markTrackInactive).toHaveBeenNthCalledWith(4, "guild-lyrics-cache");
  });

  it("should wire Genius config into created players", async () => {
    const stubProvider: LyricsProvider = {
      getLyricsForTrack: () => Promise.resolve({ status: "not_found" as const }),
    };
    const lunacord = new Lunacord({
      ...BASE_OPTIONS,
      lyrics: stubProvider,
  ...
  it("should mark restored current tracks as active for lyrics tracking", async () => {
    ...
    expect(lyricsClient.markTrackActive).toHaveBeenCalledWith("guild-restore-lyrics", track);
  });
  ...
  it("should emit pluginError for observe failures on node-less events", async () => {
    ...
    lunacord.on("pluginError", ({ plugin, hook, error, nodeId }) => {
      pluginErrors.push(`${plugin.name}:${hook}:${error.message}:${nodeId ? "node" : "none"}`);
    });

    const emitObserved = (
      lunacord as unknown as {
        emitObserved: (type: "nodeCreate", payload: { node: Node }) => void;
      }
    ).emitObserved;
    emitObserved.call(lunacord, "nodeCreate", {
      node: lunacord.getNode("node-a")!,
    });
    await Promise.resolve();

    expect(pluginErrors).toContain("failing-observer:observe:observer failed:node");
  });
  ...
  it("should let plugins intercept REST requests for managed nodes", async () => {
    ...
    lunacord.use({
      ...
      beforeRestRequest: (context) => {
        requests.push(`${context.nodeId}:${context.path}`);
        return undefined;
      },
    });
