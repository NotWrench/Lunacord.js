...
import type { PlayerNodeAdapter, SearchResult } from "@lunacord/core";
import {
  Filter,
  type Filters,
...
import type { LyricsClient } from "@lunacord/lyrics";
...
describe("Player", () => {
  ...
  describe("seek", () => {
    ...
    it("should throw when there is no current track", async () => {
      await expect(player.seek(1000)).rejects.toBeInstanceOf(InvalidPlayerStateError);
    });
  });
    ...
    it("should return the current synced lyric line", () => {
      ...
      const line = player.getCurrentLyricLine({
        ...
        lyrics: {
          ...
          lyricsText: "Line 1\nLine 2",
          syncedLyrics: [
            { timeMs: 4000, text: "Line 1" },
            { timeMs: 6000, text: "Line 2" },
          ],
        },
      });
  ...
  describe("estimated position", () => {
    it("should interpolate playback position while playing", () => {
      ...
      expect(player.getEstimatedPosition()).toBe(7000);
    ...
    it("should not advance while paused", () => {
      ...
      expect(player.getEstimatedPosition()).toBe(5000);
      nowSpy.mockRestore();
    });
    ...
    it("should apply timescale speed when estimating position", async () => {
      ...
      expect(player.getEstimatedPosition()).toBe(9000);
      nowSpy.mockRestore();
    });
    ...
    it("should apply slower timescale speed when estimating position", async () => {
      ...
      expect(player.getEstimatedPosition()).toBe(6000);
      nowSpy.mockRestore();
    });
