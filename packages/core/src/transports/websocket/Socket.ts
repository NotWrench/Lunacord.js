import { WebSocketMessageSchema } from "../../schemas/lavalink";
import type { PlayerUpdate, ReadyPayload, Stats, TrackEvent } from "../../types";
...
interface SocketOptions {
  ...
  /** Accepts a number or getter; resolved at connect time so adapters can bind identity late. */
  numShards: number | (() => number | undefined);
  password: string;
  port: number;
  secure?: boolean;
  /** Accepts a string or getter; resolved at connect time so adapters can bind identity late. */
...
export class Socket extends TypedEventEmitter<SocketEvents> {
  ...
  private openConnection(): void {
    ...
    const resolvedUserId = typeof userId === "function" ? userId() : userId;
    const resolvedNumShards = typeof numShards === "function" ? numShards() : numShards;

    if (!resolvedUserId) {
      this.emit(
        "error",
        new Error(
          "Lunacord identity is not bound (userId missing). Call Lunacord.bindIdentity({ userId, numShards }) or pass them in the constructor."
        )
      );
      return;
    }
    if (typeof resolvedNumShards !== "number") {
      this.emit(
        "error",
        new Error(
          "Lunacord identity is not bound (numShards missing). Call Lunacord.bindIdentity({ userId, numShards }) or pass them in the constructor."
        )
      );
      return;
    }
    ...
    const headers: Record<string, string> = {
      ...
      "Num-Shards": String(resolvedNumShards),
