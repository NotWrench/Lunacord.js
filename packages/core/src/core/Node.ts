...
import { Rest } from "../transports/rest/Rest";
import { Socket, type WebSocketFactory } from "../transports/websocket/Socket";
import type {
  Exception,
  PlayerState,
  PlayerUpdatePayload,
...
export interface NodeOptions {
  ...
  /**
   * Lyrics provider. Pass a concrete provider, or a getter function so the value can be
   * late-bound (Lunacord uses a getter so `lunacord.lyrics(provider)` takes effect even
   * after the node was constructed).
   */
  lyricsClient?: LyricsProvider | (() => LyricsProvider | undefined);
  maxReconnectAttempts?: number;
  maxReconnectDelayMs?: number;
  /** Resolved at connect time — accepts a getter so identity can be bound after construction. */
...
export class Node extends TypedEventEmitter<NodeEvents> {
  ...
  get lyricsClient(): LyricsProvider | undefined {
    const source = this.options.lyricsClient;
  ...
  private handleTrackEvent(event: TrackEvent): void {
    ...
    switch (event.type) {
      ...
      case "TrackEndEvent": {
        ...
        this.handleQueueAdvance(player, event.reason, track).catch((error) => {
  ...
  private handleVoiceServerPacket(packetData: Record<string, unknown>): void {
    ...
    this.trySyncVoiceState(guildId).catch(() => {
      /* error emitted internally */
    });
  }
