...
import { LUNACORD_PLUGIN_API_VERSION } from "../types";
...
export class PluginManager {
  ...
  dispatch(event: LunacordPluginEvent): void {
    for (const state of this.plugins) {
      void this.runHook(state, "observe", () => state.plugin.observe?.(event, state.context), {
  ...
  ): Promise<{ body?: unknown; method?: string; path?: string } | void> {
    let patch: {
      body?: unknown;
      method?: string;
      path?: string;
